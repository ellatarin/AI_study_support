...
### R10 — Long subtopics get read again

A subtopic that runs much past 700 words of speech is usually more than one step, and it may be several. When one does, read that subtopic again and find every place in it where the lecturer finishes with one thing and takes up the next. Split at all of them, not only the first. Then look at the pieces: any piece still much past 700 words gets the same treatment again.
...
- **R10**: is any subtopic much past 700 words? If so, have you read it again and split at every place the lecturer moves on, including the pieces that are themselves still long?
