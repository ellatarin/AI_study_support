...
- [ ] **A16.6** **PARKED to COLLAB.** `manifest.currentPipelineCost` drops a stage whose cost lookup failed rather than recording it as unresolved: `recomputeCost` returns the accumulator unchanged, so the stage leaves no row in cost-report section 1 and contributes nothing to the total the orphan-deletion prompt quotes before an irreversible delete. The same defect as A16.2, parked because it changes the persisted manifest and because what section 1's row should read is not settled by TD §7 — found while fixing A16.2, in neither review
